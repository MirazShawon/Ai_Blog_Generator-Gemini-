...
export default function DraftsPage() {
  ...
  // Force refresh on component mount and when navigating back to this page
  useEffect(() => {
    setLoading(true);
    fetchDrafts();
  }, [])
  ...
  const fetchDrafts = async () => {
    console.log('📥 Fetching drafts...', { searchQuery, writingPhase });
    setLoading(true);
    ...
    try {
      ...
      // Only add writing phase if it's not "all"
      if (writingPhase && writingPhase !== "all") {
        queryParams.set('writingPhase', writingPhase);
      }
      
      const url = `/api/draftfetch${queryParams.toString() ? '?' + queryParams.toString() : ''}`;
      ...
      console.log('✅ Drafts loaded:', data.length);
      setDrafts(data);
