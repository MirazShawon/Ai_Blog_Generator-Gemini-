...
export default function EditorPage({ params }: { params: { id: string } }) {
  ...
  const generateInitialContent = async (title: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        let errorMessage = 'Failed to generate initial content';
        try {
      ...
      try {
        ...
        if (!response.ok) {
          if (response.status === 404 && retryCount < maxRetries) {
            console.log(`Draft API not ready, retrying... (${retryCount + 1}/${maxRetries})`);
            retryCount++;
        ...
        toast({
          title: "Draft saved",
          description: `Your draft has been saved in "${status}" phase. Redirecting to drafts...`,
          duration: 2000,
        });

        // Redirect to drafts page to show the saved draft
        setTimeout(() => {
          router.push('/drafts');
        }, 1500);
        ...
        console.error('Draft save attempt failed:', error);
        if (retryCount < maxRetries && (error instanceof Error && error.message.includes('404'))) {
          retryCount++;
          console.log(`Retrying draft save... (${retryCount}/${maxRetries})`);
          await new Promise(resolve => setTimeout(resolve, 1000));
          return attemptSave();
        }
        throw error;
  ...
  const handleAIImprove = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        let errorMessage = 'Failed to improve content';
        try {
          const contentType = response.headers.get('content-type');
          if (contentType && contentType.includes('application/json')) {
            const error = await response.json();
  ...
  return (
    <div className="container mx-auto p-4 sm:p-6 md:p-8">
      ...
      <div className="mb-4 overflow-hidden rounded-md border shadow-sm transition-all duration-300 hover:shadow-md">
